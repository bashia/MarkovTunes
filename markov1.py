...
def main():
	...
	makemidi(midilist, 0) 
	makemarkov(midilist)
	w = wave.open('./bday.wav', 'r')
	length = w.getnframes()
	wholefile = []
	for i in range(0,length):
		waveData = w.readframes(1)
		data = struct.unpack("<h", waveData)
		wholefile.append(int(data[0]))

	soundout = open('stratch.wav', 'w')

	it = stretch(wholefile, 2, 256, 1024)
	it = numpy.asarray(it)
	scipy.io.wavfile.write('stratch.wav',rate,data2)
...
def stretch(sound_array, f, window_size, h):
    ...
    result = numpy.zeros( len(sound_array) /f + window_size)
